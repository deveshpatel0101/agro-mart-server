...
describe('/public/shared', () => {
    ...
    beforeEach(async() => {
        ...
        let result = await request(server).post('/user/blogs?token=' + token).send(createBlog());
    ...
    describe('PUT /', () => {
        ...
        const exec = async() => {
            let result = await request(server).put('/public/shared?token=' + token).send(blog);
            expect(result.body.error).toBeTruthy();
            expect(result.body.errorType).toBe(errorType);
        }

        const checkShared = async() => {
            await request(server).put('/public/shared?token=' + token).send(blog);
        ...
        it('should do nothing if current value of shared is passed to update again', async() => {
            ...
            result = await request(server).put('/public/shared?token=' + token).send(blog);
            expect(result.body.message).toEqual(expect.stringMatching(/updates required/gi));
        });
    ...
    describe('GET /', () => {
        ...
        beforeEach(async() => {
            const { body } = await request(server).post('/user/login').send({ email: originalUser.email, password: originalUser.password });
            blogId = id;
            await request(server).put('/public/shared?token=' + body.jwtToken).send({ blogId: id, values: { shared: true } });
        });

        it('should return error if invalid or no blogId is passed in parameter', async() => {
            let result = await request(server).get('/public/shared/blog').send();
            expect(result.body.error).toBeTruthy();
            result = await request(server).get('/public/shared/blog?blogId=').send();
            expect(result.body.error).toBeTruthy();
        });

        it('should return no error if valid blogId is passed in parameter', async() => {
            let result = await request(server).get('/public/shared/blog?blogId=' + blogId).send();
            expect(result.body.error).toBeFalsy();
